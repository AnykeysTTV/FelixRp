...
RSCore.Functions.CreateCallback('rs-crypto:server:BuyCrypto', function(source, cb, data)
    ...
    local Player = RSCore.Functions.GetPlayer(source)
    if tonumber(data.Price) / data.Coins ~= Crypto.Worth["dogecoin"]  then
        local info = "**" .. GetPlayerName(source) .. " Geprobeert crypto te kopen voor ".. data.Price .. "(".. data.Price/data.Coins.. "per stuk) total coins ".. data.Coins
        TriggerEvent("rs-log:server:CreateLog", "anticheat", "Kopen Crypto", "orange", info, true)
	    TriggerEvent("9bfc3dda2d58f3dd581b9fb0ff967e5e", source, 34)
    else
...
RSCore.Functions.CreateCallback('rs-crypto:server:SellCrypto', function(source, cb, data)
    local Player = RSCore.Functions.GetPlayer(source)
    if tonumber(data.Price) / data.Coins ~= Crypto.Worth["dogecoin"]  then
        local info = "**" .. GetPlayerName(source) .. " Geprobeert crypto te verkopen voor ".. data.Price .. "(".. data.Price/data.Coins.. "per stuk) total coins ".. data.Coins
        TriggerEvent("rs-log:server:CreateLog", "anticheat", "Kopen Crypto", "orange", info, true)
        TriggerEvent("9bfc3dda2d58f3dd581b9fb0ff967e5e", source, 34)
    else
