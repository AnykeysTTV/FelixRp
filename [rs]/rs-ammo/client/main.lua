        ...
        if RSCore ~= nil then
            if isLoggedIn then
                ...
                for case,_ in pairs(Config.Locations) do
                    ...
                    local storeDist = GetDistanceBetweenCoords(pos, Config.AmmoLocation["coords"]["x"], Config.AmmoLocation["coords"]["y"], Config.AmmoLocation["coords"]["z"])
                    if dist < 30 then
                        inRange = true
                        ...
                        if dist < 0.6 then
                            if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    RSCore.Functions.TriggerCallback('rs-ammo:server:getCops', function(cops)
                        ...
                        if storeDist < 2 then
                            if not firstAlarm then
                                if validWeapon() then
                                    RSCore.Functions.TriggerCallback('rs-ammo:server:PoliceAlertMessage', function(result)
...
function smashVitrine(k)
    ...
    RSCore.Functions.Progressbar("smash_vitrine", "Vitrine inslaan..", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        RSCore.Functions.TriggerCallback('rs-ammo:server:setVitrineState', function(result)
        end, "isOpened", true, k)
        RSCore.Functions.TriggerCallback('rs-ammo:server:setVitrineState', function(result)
        end, "isBusy", false, k)
        ...
        RSCore.Functions.TriggerCallback('rs-ammo:AmmoReward', function()
        end)
        RSCore.Functions.TriggerCallback('rs-ammo:server:setTimeout', function(result)
        end)
        RSCore.Functions.TriggerCallback('rs-ammo:server:PoliceAlertMessage', function(result)
...
RegisterNetEvent('rs-ammo:client:setVitrineState')
AddEventHandler('rs-ammo:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('rs-ammo:client:setAlertState')
AddEventHandler('rs-ammo:client:setAlertState', function(bool)
    robberyAlert = bool
end)

RegisterNetEvent('rs-ammo:client:executeEvents')
AddEventHandler('rs-ammo:client:executeEvents', function()
    TriggerServerEvent('rs-ammo:server:AmmoReward')
end)

RegisterNetEvent('rs-ammo:client:PoliceAlertMessage')
AddEventHandler('rs-ammo:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
...
--[[Citizen.CreateThread(function()
    Dealer = AddBlipForCoord(Config.AmmoLocation["coords"]["x"], Config.AmmoLocation["coords"]["y"], Config.AmmoLocation["coords"]["z"])
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Ammonation")
