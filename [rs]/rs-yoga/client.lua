                ...
                if h then
                 ...
                 --   Citizen.Wait(0)
                  if not IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    local k=false
                    AddEventHandler('ao-yoga:removeStress',function()
                    ...
                    Citizen.Wait(2000)
                            if h then 
                                Citizen.Wait(800)
         TriggerServerEvent('rs-hud:Server:RelieveStress', math.random(14, 18))
        end end
    k=false 
end)

RegisterCommand('xdee', function()
    TriggerServerEvent('rs-hud:Server:UpdateStress', 100)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsEntityPlayingAnim(m,'amb@world_human_yoga@male@base','base_a',3) then 
          TriggerServerEvent('rs-hud:Server:RelieveStress', math.random(10, 15))
        end
        local h = false
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        local m=PlayerPedId()
        if IsEntityPlayingAnim(m,'amb@world_human_yoga@male@base','base_a',3) then 
        helpText("Press E to stop doing Yoga")
if IsControlJustReleased(0,38) then
