...
RSAdmin.Functions.DrawText3D = function(x, y, z, text, lines)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'Randstad Admin')
        ...
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('rs-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    RSCore.Functions.Notify('Weer is veranderd naar: '..AvailableWeatherTypes[k].label)
                end
            end
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                RSAdmin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                RSAdmin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nDruk [~g~E~s~] om dit object te verwijderen!", 2)
...
RegisterNetEvent('rs-admin:client:bringTp')
AddEventHandler('rs-admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('rs-admin:client:Freeze')
AddEventHandler('rs-admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('rs-admin:client:SendReport')
AddEventHandler('rs-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('rs-admin:server:SendReport', name, src, msg)
end)
