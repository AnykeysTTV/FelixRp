        ...
        if showTrunkPos and not inInventory then
            ...
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    RSCore.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Kofferbak")
                        ...
                        if vehicle ~= 0 and vehicle ~= nil then
                            ...
                            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                if GetVehicleDoorLockStatus(vehicle) < 2 then
                                    ...
                                    RSCore.Functions.Notify("Voertuig staat op slot..", "error")
                                    return
                                end
                            else
                                CurrentVehicle = nil
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = RSCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = RSCore.Functions.GetPlayerData().items,
        maxweight = RSCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    RSCore.Functions.Progressbar("pickupsnowball", "Sneeuwballen oprapen..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(RSCore.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', RSCore.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    RSCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', RSCore.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), aDict, aLib, 1.0)
        RSCore.Functions.Notify("Mislukt!", "error")
    end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = RSCore.Functions.GetPlayerData().items[1],
        [2] = RSCore.Functions.GetPlayerData().items[2],
        [3] = RSCore.Functions.GetPlayerData().items[3],
        [4] = RSCore.Functions.GetPlayerData().items[4],
        [5] = RSCore.Functions.GetPlayerData().items[5],
    } 
