...
function AddPoliceAlert(data) {
    ...
    if (data.alertTitle == "Assistentie collega" || data.alertTitle == "Assistentie Ambulance" || data.alertTitle == "Assistentie Doctor") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    } else if (data.alertTitle == "Poging bankoverval") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    } else if (data.alertTitle == "Gevangenis uitbraak") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    }
...
$(document).on('click', '.alert-location', function(e){
    ...
    $.post('http://rs-policealerts/SetWaypoint', JSON.stringify({
        coords: LocationData,
    }))
});