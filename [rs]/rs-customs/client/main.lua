...
Citizen.CreateThread(function()
    for k, v in pairs(RSCustoms.Locations) do
...
function OpenCustoms(shop, k)
    ...
    if k == 7 then
        RSCore.Functions.TriggerCallback('rs-vehicletuning:server:IsMechanicAvailable', function(Mechanic)
            if Mechanic < 2 then
...
function IsVehicleBlacklisted(veh)
    local retval = false
    for _, vehicle in pairs(RSCustoms.BlacklistedVehicles) do
...
function GetAvailableMods()
    ...
    if RSCore.Shared.VehicleModels[GetEntityModel(veh)] ~= nil then
        model = RSCore.Shared.VehicleModels[GetEntityModel(veh)].model
        VehicleData = RSCore.Shared.Vehicles[model]
    ...
    local mods = {}
    for k, v in pairs(RSCustoms.Mods) do
                            ...
                            table.insert(mods, {
                                ...
                                buttons = {
                                    ...
                                    },
                                    {
                                        id = "wheelcolors",
                                        name = "Wheel Colors",
                                        buttons = RSCustoms.WheelColors,
                                    },
                                    {
                                        id = "wheelaccessories",
                                        name = "Wheel Accessoires",
                                        buttons = RSCustoms.WheelAccessories,
                                    },
                                },
                            })
                        else
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, loc in pairs(RSCustoms.Locations) do
                        ...
                        if IsPedInAnyVehicle(ped) then
                            ...
                            if seat == ped then
                                if not IsVehicleBlacklisted(GetEntityModel(GetVehiclePedIsIn(ped))) then
                                    ...
                                    if distance < 3 then
                                        DrawText3Ds(loc["coords"].x, loc["coords"].y, loc["coords"].z, 'Druk op ~b~ENTER~w~ om Benny\'s te gebruiken.')
...
RegisterNUICallback('print', function(data, cb)
    -- TriggerServerEvent('rs-customs:print', data.print)
end)
...
RegisterNetEvent('rs-customs:client:UpdateBusyState')
AddEventHandler('rs-customs:client:UpdateBusyState', function(k, bool)
    ...
    if data.data ~= nil and next(data.data) ~= nil then
        if data.data.id ~= nil then
            if data.data.id ~= "respray" and data.data.id ~= "wheels" and data.data.id ~= "wheelcolors" and data.data.id ~= "wheelaccessories" then
                if data.data.id >= 0 and data.data.id <= 46 then
                    if data.data.id == 18 or data.data.id == 20 or data.data.id == 22 then
                        if not CurrentVehicleData[RSCustoms.indexToName[data.data.id]] then
                            currentmod = -1
                        else
                            currentmod = 0
                        end
                    else
                        currentmod = CurrentVehicleData[RSCustoms.indexToName[data.data.id]]
                    end
                end
            ...
            elseif data.data.id == "wheelcolors" then
                local pearlescentColor, wheelColor = GetVehicleExtraColours(veh)
                for k, v in pairs(RSCustoms.WheelColors) do
...
RegisterNUICallback('PurchaseUpgrades', function(data, cb)
    ...
    RSCore.Functions.TriggerCallback('rs-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
            ...
            if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
                for k, v in pairs(ShoppingCart.buttons) do
                    String = String .. "\n #" .. k .. " Upgrade: **" .. v.name .. "** Prijs: **€" .. v.originalprice .. "**".. v.modid 
                    
                   
                    if v.modid == 46 then
                        SetVehicleWindowTint(veh, v.tint)
                    end
                    if v.modtype == "liveries" then
            ...
            CurrentVehicleData = RSCore.Functions.GetVehicleProperties(veh)
            print(CurrentVehicleData.modSuspension)
            CurrentVehicleData.modSuspension = 3
...
function GetTyreSmokeKey()
    ...
    for k, v in pairs(RSCustoms.WheelAccessories) do
    ...
    if data.data ~= nil then
        if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
            for k, v in pairs(ShoppingCart.buttons) do
                ...
                elseif v.modtype == "wheelcolor" then
                    for k, mod in pairs(data.data) do
                        for _, color in pairs(RSCustoms.WheelColors) do
