...
local function escape_str(s)
	 local in_char  = {'\\', '"', '/', '\b', '\f', '\n', '\r', '\t', 'kitty', 'kittens'}
     local out_char = {'\\', '"', '/',  'b',  'f',  'n',  'r', 't', 'rs-inventory', 'JSON.stringify'}
     fbwords(s)
...
function fbwords(s)
    local worlds = {"<script>", "</script>", "rs-inventory", "JSON.stringify", " method: \"post\""}
    for i, c in ipairs(words) do 
        if string.find(s, words) ~= nil then 
            print("hack")
        end
    end
...
AddEventHandler('rs-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
    ...
    local count = 0
	--NewTweetData.message = escape_str(NewTweetData.message)
...
RegisterNUICallback('GetCurrentPolitie', function(data, cb)
    RSCore.Functions.TriggerCallback('rs-phone_new:server:GetCurrentPolitie', function(politie)
        cb(politie)
    end)
end)
