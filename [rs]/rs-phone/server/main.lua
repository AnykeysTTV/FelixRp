...
RSCore.Functions.CreateCallback('rs-phone_new:server:GetCurrentPolitie', function(source, cb)
    local politie = {}
    for k, v in pairs(RSCore.Functions.GetPlayers()) do
        local Player = RSCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
                table.insert(politie, {
                    name = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
...
function GenerateOwnerName()
    local names = {
        ...
        [15] = { name = "Antoon Henselijn", citizenid = "RSC091H93" },
