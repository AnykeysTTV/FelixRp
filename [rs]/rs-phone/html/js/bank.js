...
$(document).on('click', '.bank-app-account', function(e){
    ...
    RS.Phone.Notifications.Add("fas fa-university", "RSBank", "Bankrekening nr. gekopieerd!", "#badc58", 1750);
});
...
RS.Phone.Functions.DoBankOpen = function() {
    ...
    $(".bank-app-account-saldo").html(RS.Phone.Data.PlayerData.money.bank+" &euro;");
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('http://rs-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    $(".bank-app-account-saldo").html((data.NewBalance).toFixed(0) + " &euro;");
                    $(".bank-app-account-saldo").data('balance', (data.NewBalance).toFixed(0));
                    RS.Phone.Notifications.Add("fas fa-university", "RSBank", "Je hebt &euro; "+amount+",- overgemaakt!", "#badc58", 1500);
                } else {
                    RS.Phone.Notifications.Add("fas fa-university", "RSBank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
                }
                RS.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        RS.Phone.Notifications.Add("fas fa-university", "RSBank", "Vul alle velden in!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://rs-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                RS.Phone.Notifications.Add("fas fa-university", "RSBank", "Je hebt &euro;"+InvoiceData.amount+" betaald!", "#badc58", 1500);
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://rs-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        RS.Phone.Notifications.Add("fas fa-university", "RSank", "Er is geen IBAN gebonden aan dit contact!", "#badc58", 2500);
    }
    RS.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});