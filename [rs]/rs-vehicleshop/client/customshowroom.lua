...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for k, v in pairs(RSCore.Shared.Vehicles) do
        if v["shop"] == "custom" then
            for cat,_ in pairs(CustomVehicleCats) do
                if RSCore.Shared.Vehicles[k]["category"] == cat then
                    table.insert(CustomVehicleCats[cat].vehicles, RSCore.Shared.Vehicles[k])
                end
            end
        end
    end
...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for i = 1, #RSCustom.ShowroomPositions, 1 do
        local oldVehicle = GetClosestVehicle(RSCustom.ShowroomPositions[i].coords.x, RSCustom.ShowroomPositions[i].coords.y, RSCustom.ShowroomPositions[i].coords.z, 3.0, 0, 70)
        if oldVehicle ~= 0 then
            RSCore.Functions.DeleteVehicle(oldVehicle)
        end

		local model = GetHashKey(RSCustom.ShowroomPositions[i].vehicle)
		RequestModel(model)
		while not HasModelLoaded(model) do
			Citizen.Wait(0)
		end

		local veh = CreateVehicle(model, RSCustom.ShowroomPositions[i].coords.x, RSCustom.ShowroomPositions[i].coords.y, RSCustom.ShowroomPositions[i].coords.z, false, false)
		SetModelAsNoLongerNeeded(model)
		SetVehicleOnGroundProperly(veh)
		SetEntityInvincible(veh,true)
        SetEntityHeading(veh, RSCustom.ShowroomPositions[i].coords.h)
...
function SetClosestCustomVehicle()
    ...
    for id, veh in pairs(RSCustom.ShowroomPositions) do
        if current ~= nil then
            if(GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[id].coords.x, RSCustom.ShowroomPositions[id].coords.y, RSCustom.ShowroomPositions[id].coords.z, true) < dist)then
                current = id
                dist = GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[id].coords.x, RSCustom.ShowroomPositions[id].coords.y, RSCustom.ShowroomPositions[id].coords.z, true)
            end
        else
            dist = GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[id].coords.x, RSCustom.ShowroomPositions[id].coords.y, RSCustom.ShowroomPositions[id].coords.z, true)
            current = id
...
Citizen.CreateThread(function()
    while true do
        local pos = GetEntityCoords(GetPlayerPed(-1), true)
        local ShopDistance = GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[1].coords.x, RSCustom.ShowroomPositions[1].coords.y, RSCustom.ShowroomPositions[1].coords.z, false)
...
Citizen.CreateThread(function()
    while true do
        local ped = GetPlayerPed(-1)
        local pos = GetEntityCoords(ped, false)
        local dist = GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.z, false)

        if isLoggedIn then
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" then
                        DrawText3Ds(RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9, '~g~G~w~ - Voertuig veranderen')
                        DrawText3Ds(RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.75, '~b~/verkoop [id]~w~ - Voertuig verkopen ~b~/testrit~w~ - Testrit maken')
                                        ...
                                        if selected then
                                            if IsControlJustPressed(1, 18) then
                                                if CustomModelLoaded then
                                                    TriggerServerEvent('rs-vehicleshop:server:SetCustomShowroomVeh', button.model, ClosestCustomVehicle)
                                                end
                                            end
                                        end
                                    end
...
RegisterNetEvent('rs-vehicleshop:client:SetCustomShowroomVeh')
AddEventHandler('rs-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    CancelEvent()
    if RSCustom.ShowroomPositions[k].vehicle ~= showroomVehicle then
        RSCore.Functions.DeleteVehicle(GetClosestVehicle(RSCustom.ShowroomPositions[k].coords.x, RSCustom.ShowroomPositions[k].coords.y, RSCustom.ShowroomPositions[k].coords.z, 3.0, 0, 70))
        ...
        local veh = CreateVehicle(model, RSCustom.ShowroomPositions[k].coords.x, RSCustom.ShowroomPositions[k].coords.y, RSCustom.ShowroomPositions[k].coords.z, false, false)
        SetModelAsNoLongerNeeded(model)
        SetVehicleOnGroundProperly(veh)
        SetEntityInvincible(veh,true)
        SetEntityHeading(veh, RSCustom.ShowroomPositions[k].coords.h)
        SetVehicleDoorsLocked(veh, 3)

        FreezeEntityPosition(veh, true)
        SetVehicleNumberPlateText(veh, k .. "CARSALE")
        CustomModelLoaded =  true
        RSCustom.ShowroomPositions[k].vehicle = showroomVehicle
    end
end)

RegisterNetEvent('rs-vehicleshop:client:ConfirmVehicle')
AddEventHandler('rs-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
    RSCore.Functions.SpawnVehicle(Showroom.vehicle, function(veh)
...
RegisterNetEvent('rs-vehicleshop:client:DoTestrit')
AddEventHandler('rs-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        RSCore.Functions.SpawnVehicle(RSCustom.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['LegacyFuel']:SetFuel(veh, 100)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
...
RegisterNetEvent('rs-vehicleshop:client:SellCustomVehicle')
AddEventHandler('rs-vehicleshop:client:SellCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local player, distance = GetClosestPlayer()

    if player ~= -1 and distance < 2.5 then
        local VehicleDist = GetDistanceBetweenCoords(pos, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, RSCustom.ShowroomPositions[ClosestCustomVehicle].coords.z)
        
            
            if VehicleDist < 2.5 then
                TriggerServerEvent('rs-vehicleshop:server:SellCustomVehicle', TargetId, ClosestCustomVehicle)
            else
                RSCore.Functions.Notify("Je bent niet bij het voertuig in de buurt!", "error")
            end
...
RegisterNetEvent('rs-vehicleshop:client:SetVehicleBuying')
AddEventHandler('rs-vehicleshop:client:SetVehicleBuying', function(slot)
    RSCustom.ShowroomPositions[slot].buying = true
    SetTimeout((60 * 1000) * 5, function()
        RSCustom.ShowroomPositions[slot].buying = false
    end)
end)
