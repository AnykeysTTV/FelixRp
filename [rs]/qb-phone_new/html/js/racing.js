...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "The race has already started..", "#1DA1F2");
    }
});
                ...
                if (InDistance) {
                    $.post('http://qb-phone_new/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "You are in an editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are already in a race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#create-race', function(e){
    ...
    $.post('http://qb-phone_new/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
        if (data.IsAuthorized) {
            if (!data.IsBusy) {
                $.post('http://qb-phone_new/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are in a race..", "#1DA1F2");
                    }
                });
            } else {
            ...
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to create Race Tracks..", "#1DA1F2");
        }
    });
});
                        ...
                        if (laps !== "") {
                            $.post('http://qb-phone_new/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "No races can be made at the moment..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Enter an amount of rounds..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have not chosen a Track..", "#1DA1F2");
                    }
                }
            })
        } else {
