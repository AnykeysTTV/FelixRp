...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://qb-phone_new/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://qb-phone_new/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@Powerland#8704.com");
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://qb-phone_new/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You cannot place an empty AD!", "#ff8f1a", 2000);
    }
});
