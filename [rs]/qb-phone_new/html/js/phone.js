            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "أنت بالفعل مشغول");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "الشخص يتحدث");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Phone", "الشخص غير موجود");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "لايمكنك الاْتصال على رقمك");
        }
    });
});
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        ...
        $("#total-contacts").text(TotalContacts+ " جهات إتصالي");
    } else {
        $("#total-contacts").text("0 جهات إتصالي");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "لايمكنك ارسال لنفسك..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    $(".phone-edit-contact-header").text(ContactData.name+" تعديل")
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "تعديل جهة الإتصال", "قم بلمئ الفراغات");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://qb-phone_new/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://qb-phone_new/RemoveSuggestion', JSON.stringify({
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            $(".phone-currentcall-title").html("InCall ("+cData.CallTime+")");
        }
