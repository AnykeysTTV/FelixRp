...
function SetupPlayerHouses(Houses) {
    ...
    if (Houses.length > 0) {
        $.each(Houses, function(id, house){
			...
			//house.garage.length
            if (1 > 0) {
                HouseDetails = '<i class="fas fa-key"></i>&nbsp;&nbsp;' + TotalKeyholders + '&nbsp&nbsp&nbsp&nbsp<i class="fas fa-warehouse"></i>&nbsp;&nbsp;&nbsp;' + house.cars + '';
            }
...
$(document).on('click', '.house-key-delete', function(e){
    ...
    $.post('http://qb-phone_new/RemoveKeyholder', JSON.stringify({
        HolderData: Data,
...
$(document).on('click', '#myhouse-option-transfer-confirm', function(e){
    ...
    $.post('http://qb-phone_new/TransferCid', JSON.stringify({
        newBsn: NewBSN,
        HouseData: CurrentHouseData,
        ...
        if (CanTransfer) {
            ...
            QB.Phone.Notifications.Add("fas fa-home", "Estate", "رقم المعرف هذا غير صحيح", "#27ae60", 2500);
