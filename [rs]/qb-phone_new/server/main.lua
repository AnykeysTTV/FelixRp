...
RegisterServerEvent('rs-phone_new:server:AddAdvert')
AddEventHandler('rs-phone_new:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('rs-phone_new:client:UpdateAdverts', -1, Adverts, "@"..Player.PlayerData.charinfo.firstname..""..Player.PlayerData.charinfo.lastname)
end)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetCallState', function(source, cb, ContactData)
...
RegisterServerEvent('rs-phone_new:server:RemoveMail')
AddEventHandler('rs-phone_new:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('rs-phone_new:client:UpdateMails', src, mails)
        end)
    end)
end)
...
AddEventHandler('rs-phone:server:sendNewMail', function(mailData)
    ...
    TriggerClientEvent('rs-phone_new:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('rs-phone_new:server:ClearButtonData')
AddEventHandler('rs-phone_new:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('rs-phone_new:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('rs-phone_new:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

QBCore.Functions.CreateCallback('rs-phone_new:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Trgt = QBCore.Functions.GetPlayerByCitizenId(sender)
    local Invoices = {}
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetContactPictures', function(source, cb, Chats)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetContactPicture', function(source, cb, Chat)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetPicture', function(source, cb, number)
        ...
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('rs-phone_new:client:TransferMoney', recieverSteam.PlayerData.source, amount, recieverSteam.PlayerData.money.bank)
                end
            else
                local moneyInfo = json.decode(result[1].money)
            ...
            TriggerClientEvent('RSCore:Notify', src, "رقم الحساب هذا غير موجود!", "error")
        end
    end)
end)
...
RegisterServerEvent('rs-phone_new:server:RemoveContact')
AddEventHandler('rs-phone_new:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('rs-phone_new:server:AddRecentCall', function(type, data)
    ...
    TriggerClientEvent('rs-phone_new:client:AddRecentCall', src, data, label, type)

    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('rs-phone_new:client:AddRecentCall', Trgt.PlayerData.source, {
...
AddEventHandler('rs-phone_new:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:FetchResult', function(source, cb, search)
    local src = source
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
...
QBCore.Functions.CreateCallback('rs-phone_new:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
        ...
        TriggerClientEvent('RSCore:Notify', src, "لا توجد مركبة قريبة ..", "error")
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetGarageVehicles', function(source, cb)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:HasPhone', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('rs-phone_new:server:GiveContactDetails')
AddEventHandler('rs-phone_new:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('rs-phone_new:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('rs-phone_new:server:AddTransaction')
AddEventHandler('rs-phone_new:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetCurrentTow', function(source, cb)
    local tow = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetCurrentMech', function(source, cb)
    local mech = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('rs-phone_new:server:GetCurrentTaxi', function(source, cb)
    local taxi = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterServerEvent('rs-phone_new:server:InstallApplication')
AddEventHandler('rs-phone_new:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('rs-phone_new:server:RemoveInstallation', function(App)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
