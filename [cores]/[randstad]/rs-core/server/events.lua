...
-- plaats did in events.lua server side
function logCommands(steam,cmd,commands)

	local cmdlogs = steam .. " gebruikte command \\".. cmd
	
	for _, v in pairs(commands) do 
		cmdlogs = cmdlogs .." " .. tostring(v)
	end		
	TriggerEvent("rs-log:server:CreateLog", "commands", "commands", "default", cmdlogs)			
end
		...
		if RSCore.Commands.List[command] ~= nil then
			...
			if Player ~= nil then
				...
				if (RSCore.Functions.HasPermission(source, "god") or RSCore.Functions.HasPermission(source, RSCore.Commands.List[command].permission)) then
					logCommands(Player.PlayerData.name,command,  args)
					if (RSCore.Commands.List[command].argsrequired and #RSCore.Commands.List[command].arguments ~= 0 and args[#RSCore.Commands.List[command].arguments] == nil) then
						--plaats dit in addeventhandler chatmessage
...
RegisterServerEvent("rs-core:server:doPermban")
AddEventHandler("rs-core:server:doPermban", function(source)
	...
	TriggerEvent("rs-log:server:CreateLog", "bans", "Player Banned", "orange", "Speler: "..GetPlayerName(source).." verbannen doormiddel van score (flags).")

	RSCore.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`) VALUES ('"..GetPlayerName(source).."', '"..GetPlayerIdentifiers(source)[1].."', '"..GetPlayerIdentifiers(source)[2].."', '"..GetPlayerIdentifiers(source)[3].."', '"..GetPlayerIdentifiers(source)[4].."', 'Hacking', "..banTime..")")
	DropPlayer(source, "Hé kansloos, lekker terug naar esx server =p\n\nJe ban verloopt "..timeTable["day"].. "/" .. timeTable["month"] .. "/" .. timeTable["year"] .. " " .. timeTable["hour"].. ":" .. timeTable["min"] .. "\n🔸 Kijk op onze discord voor meer informatie")
